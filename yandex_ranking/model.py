...
class Session(object):
    ...
    def gen_hash_feature(self, n_features=2 ** 10):
        """
        generate feature using hashing trick. use items before last click.
        :param:
    ...
    def gen_k_hash_feature(self, n_features=2 ** 10, k=10):
        """
        generate feature using hashing trick. use top-k items.
        :param:
        n_features: feature dimension
        k: top-k items to use.
        :return:
        x: numpy.2darray
        y: numpy.1darray
        """
...
if __name__ == '__main__':
    ...
    x, y = sessions[0].gen_k_hash_feature(n_features=8, k=10)
    if x is not None:
        print(x, x.shape)
        print(y, y.shape)
        print()
