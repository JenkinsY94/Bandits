...
# ++++++++++++++++++++++++++++++++++++
sessions = []
sess_train_1, sess_train_2, sess_explore, sess_test = [], [], [], []
...
# split all sessions into 4 parts
# day 1-6 for train Model 1, day 7-9 for train model 2,
# day 10-19 for explore and retrain (for baseline this is for train only),
# day 20-30 for test.
for s in sessions:
    if s.day < 7:
        sess_train_1.append(s)
    elif s.day < 10:
        sess_train_2.append(s)
    elif s.day < 20:
...
print("proportion of samples in: \ntrain set_1: %.3f%%, \ntrain set_2: %.3f%%, \nexplore set: %.3f%%, \ntest set: %.3f%%\n"
      % (100.0*len(sess_train_1)/num_sessions, 100.0*len(sess_train_2)/num_sessions, 100.0*len(sess_explore)/num_sessions, 100.0*len(sess_test)/num_sessions))
...
X_train_1, y_train_1 = construct_feature(sess_train_1, k=K)  # training samples for model 1
X_train_2, y_train_2 = construct_feature(sess_train_2, k=K)   # training samples for model 2

print("(LR)Shape of X_train: %s, shape of y_train: %s" % (X_train_1.shape, y_train_1.shape))
print("(GBDT)Shape of X_train: %s, shape of y_train: %s" % (X_train_2.shape, y_train_2.shape))
# train model 1
clf_1 = LogisticRegression(penalty='l1', solver='liblinear', C=1.0, class_weight='balanced', verbose=0)
clf_1.fit(X_train_1, y_train_1)
# train model 2
assert TS_POLICY == 'abs_err'
...
ts = TSOverAbsErr()  # init Thompson sampling class. ** without prior. **
...
new_y = np.concatenate((y_train_1, y_train_2, y_explore))
sample_weight_train = [1 for i in range(len(y_train_1)+len(y_train_2))]
